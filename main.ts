...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000016-card-sorting",
    task_name: "Card Sorting Task (WCST-style)",
    task_description: "HTML preview aligned to local psyflow Card Sorting procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
      ...
      for (let blockIndex = 0; blockIndex < totalBlocks; blockIndex += 1) {
        ...
        const rule = String(configuredRules[blockIndex % configuredRules.length] ?? "color");
          ...
          buildWaitTrial(
            ...
            [
              (_snapshot: TrialSnapshot, runtime: RuntimeView) => {
                ...
                return stimBank.get_and_format(`block_break_${rule}`, {
          ...
          [
            (_snapshot: TrialSnapshot, runtime: RuntimeView) =>
              stimBank.get_and_format("good_bye", {
                total_score: summarizeTotalScore(runtime.getReducedRows())
              })
          ]
        )
      );
